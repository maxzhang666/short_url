			...
			<?php if ($do == '0') { ?>
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						<p><iframe src="../readme.txt" style="width:100%;height:150px;"></iframe></p>
						<?php if ($installed) { ?>
							<div class="alert alert-warning">您已经安装过，如需重新安装请删除<font color=red> install/dwz </font>文件后再安装！</div>
						<?php } else { ?>
						...
						<?php
							...
							if (defined("SAE_ACCESSKEY") || $_GET['jump'] == 1) {
								...
								if (!$username || !$password || !$dbname) {
									...
									if (!$con = DB::connect($servername, $username, $password, $dbname, $dbport)) {
										...
										if (DB::query("select * from config where 1") == FALSE)
											...
											echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过5G云短网址程序</div>
...
?>";
					...
					<div class="panel-body">
						<?php
							...
							if (!$username || !$password || !$dbname) {
								...
								$sql = explode(';', $sql);
								$cn = DB::connect($servername, $username, $password, $dbname, $dbport);
			...
			<?php } elseif ($do == '5') { ?>
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						<?php
							@file_put_contents("dwz", '');
							echo '<div class="alert alert-info"><font color="green">安装完成！管理账号和密码是:admin/123456</font><br/><br/><a href="../">>>网站首页</a>｜<a href="../admin/">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 dwz 文件！</font></div>';
							?>
					</div>
				</div>
