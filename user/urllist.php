    ...
    <section class="wrapper">
        ...
        <?php
            ...
            if ($url == NULL) {
                ...
                $tcn = tcn($tzurl);
                $urlcn = urlcn($tzurl);
                $sql = "insert into url_list(id,uid,addtime,tcn,urlcn,url,qqjump,wxjump,alijump,pwd,setip,remarks) values('$id','$uid','$nowDate','$tcn','$urlcn','$url','$qqjump','$wxjump','$alijump','$pwd','$ip','$remarks')";
                $rs = $conn->query($sql);
                if ($rs) {
            ...
            if ($rs->num_rows == 0) {
                ...
                $alijump = trim($_POST['alijump']);
                if ($url == NULL) {
                    showmsg('保存错误,跳转网址不能为空!', 3);
                } else {
                    $rs = $conn->query("update url_list set url='$url',remarks='$remarks',pwd='$pwd',qqjump='$qqjump',wxjump='$wxjump',alijump='$alijump' where id='$id' and uid='$uid'");
                    if ($rs) {
                            ...
                            <form action="./urllist.php?my=edit_submit&id=<?php echo $row['id']; ?>" method="post" class="cmxform form-horizontal tasi-form" role="form">
                                <div class="form-group">
                                    ...
                                    <div class="input-group col-sm-10">
                                        <input type="text" name="tcn" value="<?php echo $row['tcn']; ?>" class="form-control" readonly />
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group col-sm-10">
                                        <input type="text" name="urlcn" value="<?php echo $row['urlcn']; ?>" class="form-control" readonly />
                                        <span class="input-group-btn">
        ...
        <?php
            ...
            if ($rs->num_rows == 0) {
                ...
                $rs = $conn->query("delete from url_list where id='$id' and uid = '$uid'");
                if ($rs) {
            ...
            success: function(data) {
                if (data.code == 0) {
                    layer.open({
                        ...
                        shadeClose: true,
                        title: '详细信息',
                        content: '<div style="padding:15px"><p>默认跳转：' + data.url + '</p><p>网址备注：' + data.remarks + '</p><p>新浪短链：' + data.tcn + '</p><p>腾讯短链：' + data.urlcn + '</p><p>QQ跳转：' + data.qqjump + '</p><p>微信跳转：' + data.wxjump + '</p><p>支付宝跳转：' + data.alijump + '</p><p>访问次数：' + data.view + '</p><p>访问密码：' + data.pwd + '</p><p>添加时间：' + data.addtime + '</p></div>'
                    });
                } else {
