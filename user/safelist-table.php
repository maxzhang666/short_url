...
if (isset($_GET['id'])) {
    ...
    $sql = " (url='{$_GET['kw']}' or id='{$_GET['kw']}')";
    $link = '&kw=' . $_GET['kw'];
} else {
    $sql = "1";
}
$rs = $conn->query("select * from url_safe where({$sql} and uid='$uid')");
$numrows = $rs->num_rows;
?>
